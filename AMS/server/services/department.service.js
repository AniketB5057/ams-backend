...
const createDepartment = async (req) => {
  ...
  try {
    ...
    const department = await models.department.create({ name, departmentUniqueId, userId: createdBy, createdBy });
...
const updateDepartment = async (req) => {
  ...
  try {
    ...
    if (!department) {
      ...
      await department.update({ name, updatedBy });
    }
...
const getallDepartment = async (req) => {
  ...
  try {
    ...
    const departmentData = await models.department.findAndCountAll();
    if (!departmentData) {
      throw new Error("department not found")
    } else {
...
const deleteDepartment = async (departmentId) => {
  ...
  try {
    ...
    let statuschange;
    if (departmentDatae.isActive == 0) {
      statuschange = 1;
    } else {
      statuschange = 0;
    }
    let departmentData = await models.department.update({ isActive: statuschange }, { where: { id: departmentId } });
